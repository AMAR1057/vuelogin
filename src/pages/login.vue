  ...
  >
    ...
    >
      ...
      <form @submit.prevent="logginIn" class="flex flex-col p-4 space-y-4">
        ...
        <span class= "text-red-600 text-center text-xs">{{emailError}}</span>
        <input 
        name="password"
          type="password"
          class="p-2 border-2 rounded-lg"
          ...
          v-model="password"
        />
        <span class= "text-red-600 text-center text-xs">{{passwordError}}</span>
        <div class="flex space-x-2">
          <button
            type="submit"
            @submit.prevent="logginIn"
            class="w-1/2 py-2 text-pink-200 bg-purple-600 rounded-lg"
          >
            Login
          </button>
          <button
        ...
        <button @click="google"
        class=" flex justify-center py-2 bg-white rounded-lg hover:bg-gray-300"><img src="https://upload.wikimedia.org/wikipedia/commons/5/53/Google_%22G%22_Logo.svg" alt=""></button>
      </form>
